...
Flux.subsystem_enrichment_list <- function(sigflux,annote,outtype = 'matrix'){
  require(clusterProfiler)
  fluxgmt = data.frame(ont = annote$subSystemes,gene = rownames(annote),stringsAsFactors = F) 
...
one_line_flux_analysis <- function(flux3, fluxCase, fluxControl,flux3.annote,outdir,pcutoff = 0.05){
    #bootstrap
    if(!dir.exists(outdir)){
      dir.create(outdir)
    }
    print("Step 1: DEflux and bootstrap")
    flux.CI = cbind(fluxCase,fluxControl)
    clin.CI = data.frame(ids = colnames(flux.CI),stringsAsFactors = F,
                        class = c(rep('CI',ncol(fluxCase)),rep('Control',ncol(fluxControl)))
                         )
    
    
    DEflux.CI = DEGenes.simplified(flux.CI,catagory = clin.CI$class == 'CI')
    DEflux.CI = cbind(DEflux.CI,flux3.annote)
    print(paste0("N cases:    ",ncol(fluxCase)))
    print(paste0("N control: ",ncol(fluxControl)))
    print(paste0("N up-regulated rxns:   ",sum(DEflux.CI$Pvalue < pcutoff & DEflux.CI$log2FC > 0)))
    print(paste0("N down-regulated rxns: ",sum(DEflux.CI$Pvalue < pcutoff & DEflux.CI$log2FC < 0)))
    
    nboot = 1000
    nrxns = nrow(flux.CI)
    bootstrapDEflux.CI = list()
    bootstrapDEflux.CI[[1]] = matrix(0,nrxns,nboot)  # log2FC
    bootstrapDEflux.CI[[2]] = matrix(1,nrxns,nboot)  # Pvalue
    names(bootstrapDEflux.CI) = c('log2FC','Pvalue')
    for (k in 1:nboot){
    #randperm
        clin.CI$RandClass = clin.CI$class[randperm(nrow(clin.CI))]
        tmpde = DEGenes.simplified(flux.CI,catagory = clin.CI$RandClass == 'CI')
        bootstrapDEflux.CI[[1]][,k] = tmpde$log2FC
        bootstrapDEflux.CI[[2]][,k] = tmpde$Pvalue
    }
      
    #save(list = c('flux.CI','DEflux.CI','flux3.annote','clin.CI','bootstrapDEflux.CI'),
    #     file = paste0(outdir,'/Flux3_bootstrap.Rdata'))
    writetxt(DEflux.CI,paste0(outdir,'/DEflux.txt'))
    print("Step1 Done")
    
    print("Step2 pathway analysis")
    ## step 2 WDA pathway analysis
    WDA.cen.bootstrap.CI = Flux.subsystem_WDA_bootstrap(DEflux.CI,bootstrapDEflux.CI[[1]], bootstrapDEflux.CI[[2]],
                                       flux3.annote,cutoff.nrxns = 2,cutoff.pval =pcutoff,cutoff.fc = 0)
    writetxt(WDA.cen.bootstrap.CI,paste0(outdir,'/WDA.cen.bootstrap.txt'))
    
    #plot transport
    pdf(paste0(outdir,'/Figure_DEflux_bootstrap_transport.pdf'),width = 11,height =8)
    idaa = substr(WDA.cen.bootstrap.CI$subsystem,1,9) == 'Transport'
    print(plot_DAscore(WDA.cen.bootstrap.CI$DAscore[idaa],WDA.cen.bootstrap.CI$subsystem[idaa],enlarge = 1.8,gtitle = 'Transport fluxes',
                   #lengend.position = "top", 
                   WDA.cen.bootstrap.CI$fdr[idaa],gsize =28,gsizex = 20,gsizey = 24))
    dev.off()
    
    # plot enzymatic
    pdf(paste0(outdir,'/Figure_DEflux_bootstrap_enzymatic_reactions.pdf'),width = 20,height =18)
    idaa = substr(WDA.cen.bootstrap.CI$subsystem,1,9) != 'Transport' & WDA.cen.bootstrap.CI$subsystem != 'Exchange/demand reaction' & WDA.cen.bootstrap.CI$fdr < 0.2
    print(plot_DAscore(WDA.cen.bootstrap.CI$DAscore[idaa],WDA.cen.bootstrap.CI$subsystem[idaa],enlarge = 3,gtitle = 'Enzymatic reactions',
                   WDA.cen.bootstrap.CI$fdr[idaa],gsize =30,gsizex = 26,gsizey = 28))
    dev.off()
    
    #uptake
    idaa = DEflux.CI$subSystemes == 'Exchange/demand reaction' & rowMeans(flux3[,colnames(flux.CI)]) < 0 & substr(rownames(DEflux.CI),1,2) == 'EX' & rowMeans(flux.CI) > log2(1e-5)

    res2 = DEflux.CI[idaa,]
    keyvals <- rep('gray50', nrow(res2))
    names(keyvals) <- rep('NS', nrow(res2))   

    keyvals[which(res2$log2FC > 0 & res2$Pvalue < 0.05)] <- "Brown"    
    names(keyvals)[which(res2$log2FC > 0 & res2$Pvalue < 0.05)] <- 'High'    

    keyvals[which(res2$log2FC < -0 & res2$Pvalue < 0.05)] <- "darkblue"    
    names(keyvals)[which(res2$log2FC < -0 & res2$Pvalue < 0.05)] <- 'Low'   
    
    #p = EnhancedVolcano(res2, lab = res2$ID,   x = 'log2FC', pCutoff = 0.05,cutoffLineCol = 'white',  colAlpha = 0.9,
    ...
    #            gridlines.minor = F,gridlines.major = F,
     #           subtitle = NULL,transcriptPointSize = -2*log10(res2$Pvalue),transcriptLabSize = 6,boxedlabels = F,
    #            y = 'Pvalue',     xlab = "log2FC ", colCustom = keyvals)
    p = EnhancedVolcano(res2,
                        ...
                        drawConnectors = TRUE,
                        widthConnectors = 1.0,
                        pointSize  = -2*log10(res2$Pvalue),labSize  = 6,
                        colConnectors = 'black')
    pdf(paste0(outdir,'/Figure_DEflux_bootstrap_uptake.pdf'),width = 10,height =10)
    print(p)
    dev.off()
    ...
    #p= EnhancedVolcano(res2, lab = res2$ID,   x = 'log2FC', pCutoff = 0.05,cutoffLineCol = 'white',  colAlpha = 0.9,
    #            legendPosition = 'botton',
    #            title = 'Secretion  fluxes',border = 'full',titleLabSize = 28,
    #            FCcutoff = 0,cutoffLineWidth = 0,axisLabSize = 28,
    #            gridlines.minor = F,gridlines.major = F,
    #            subtitle = NULL,transcriptPointSize = -2*log10(res2$Pvalue),transcriptLabSize = 6,boxedlabels = F,
    #            y = 'Pvalue',     xlab = "log2FC ", colCustom = keyvals)
    p = EnhancedVolcano(res2,
                        ...
                        pointSize  = -2*log10(res2$Pvalue),labSize  = 6,
                        colConnectors = 'black')
    pdf(paste0(outdir,'/Figure_DEflux_bootstrap_secretion.pdf'),width = 10,height =10)
    print(p)
    dev.off()
    print("Step2 down")
    
    save(list = c('flux.CI','DEflux.CI','flux3.annote','clin.CI','bootstrapDEflux.CI','WDA.cen.bootstrap.CI'),
         file = paste0(outdir,'/Flux3_one_line_data.Rdata'))
    return(WDA.cen.bootstrap.CI)
}
